...
class AttestationInfo(BaseModel):
    ...
    signature_present: bool = Field(default=False, description="Whether signature is present")
    signature_confidence: float = Field(default=0.0, ge=0.0, le=1.0, description="Signature detection confidence")
    name: AttestationField = Field(default_factory=AttestationField, description="Prescriber name on attestation")
    date: AttestationField = Field(default_factory=AttestationField, description="Attestation date")
    ...
    def is_valid(self) -> bool:
        ...
        return (
            ...
            and self.name.validated
            and self.date.validated
        )
