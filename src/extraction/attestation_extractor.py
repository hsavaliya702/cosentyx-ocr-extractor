...
class AttestationExtractor(BaseExtractor):
    ...
    FIELD_MAPPINGS = {
        'name': [
            'prescriber signature', 'signature name', 'signed by',
            'prescriber name', 'physician name', 'attestation name'
        ],
        'date': [
            'signature date', 'date signed', 'attestation date', 'sign date'
        ]
    }

    def extract(self, textract_data: Dict, payload_data: Dict = None) -> AttestationInfo:
        ...
        logger.info(f"Attestation extraction complete. Valid: {attestation_info.is_valid()}")
    ...
    def _extract_name(self, forms: Dict, payload_data: Dict = None) -> AttestationField:
        ...
        if not value and payload_data:
            value = self.get_from_payload(payload_data, ['attestation_name', 'prescriber_name'])
            if value:
                source = "payload"
                confidence = 1.0
    ...
    def _extract_date(self, forms: Dict, payload_data: Dict = None) -> AttestationField:
        ...
        # Try payload
        if not value and payload_data:
            value = self.get_from_payload(payload_data, ['attestation_date', 'signature_date'])
            if value:
                source = "payload"
                confidence = 1.0
                original_value = value
