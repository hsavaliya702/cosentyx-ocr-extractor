...
class PatientExtractor(BaseExtractor):
    ...
    FIELD_MAPPINGS = {
        ...
        "gender": ["sex", "gender", "patient gender", "patient sex", "member gender"],
        "phone": [
            "phone",
            "telephone",
            "phone number",
    ...
    def extract(self, textract_data: Dict, payload_data: Dict = None) -> PatientInfo:
        ...
        patient_info.preferred_language = self._extract_preferred_language(
            forms, payload_data
        )
    ...
    def _extract_first_name(
        ...
        if not value and payload_data:
            value = self.get_from_payload(
                payload_data, self.FIELD_MAPPINGS["first_name"]
            )
            if value:
                source = "payload"
                confidence = 1.0
    ...
    def _extract_last_name(
        ...
        if not value and payload_data:
            value = self.get_from_payload(
                payload_data, self.FIELD_MAPPINGS["last_name"]
            )
            if value:
                source = "payload"
                confidence = 1.0
    ...
    def _extract_preferred_language(
        self, forms: Dict, payload_data: Dict = None
    ) -> PatientField:
        """Extract preferred language (optional)."""
        value, confidence = self.find_field(
            forms, self.FIELD_MAPPINGS["preferred_language"]
        )
        source = "form"
        ...
        if not value and payload_data:
            value = self.get_from_payload(
                payload_data, self.FIELD_MAPPINGS["preferred_language"]
            )
            if value:
                source = "payload"
                confidence = 1.0
