...
class PrescriptionExtractor(BaseExtractor):
    ...
    FIELD_MAPPINGS = {
        'product': [
            'product', 'drug', 'medication', 'drug name', 'product name',
            'cosentyx', 'medicine'
        ],
        'dosage': [
            'dosage', 'dose', 'strength', 'drug strength', 'product strength'
        ],
        'quantity': [
            'quantity', 'qty', 'amount', 'number of units', 'units'
        ],
        'sig': [
            'sig', 'directions', 'instructions', 'directions for use',
            'how to use', 'administration'
        ],
        'refills': [
            'refills', 'number of refills', 'refill', 'rx refills'
        ]
    }

    def extract(self, textract_data: Dict, payload_data: Dict = None) -> PrescriptionInfo:
        ...
        logger.info(f"Prescription extraction complete. Valid: {prescription_info.is_valid()}")
        return prescription_info

    def _extract_product(self, forms: Dict, payload_data: Dict = None) -> PrescriptionField:
    ...
    def _extract_dosage(self, forms: Dict, payload_data: Dict = None) -> PrescriptionField:
    ...
    def _extract_quantity(self, forms: Dict, payload_data: Dict = None) -> PrescriptionField:
    ...
    def _extract_sig(self, forms: Dict, payload_data: Dict = None) -> PrescriptionField:
        ...
        # Try payload
        if not value and payload_data:
            value = self.get_from_payload(payload_data, ['sig', 'directions', 'instructions'])
            if value:
                source = "payload"
                confidence = 1.0
    ...
    def _extract_refills(self, forms: Dict, payload_data: Dict = None) -> PrescriptionField:
