...
class PrescriberExtractor(BaseExtractor):
    ...
    FIELD_MAPPINGS = {
        ...
        "city": ["city", "prescriber city", "physician city", "office city"],
        "state": ["state", "prescriber state", "physician state", "office state"],
        "zip": [
            "zip",
            "zip code",
            "zipcode",
        ...
        "fax": ["fax", "fax number", "prescriber fax", "physician fax", "office fax"],
    }
    ...
    def extract(self, textract_data: Dict, payload_data: Dict = None) -> PrescriberInfo:
        ...
        logger.info(
            f"Prescriber extraction complete. Valid: {prescriber_info.is_valid()}"
        )
    ...
    def _extract_address(self, forms: Dict, payload_data: Dict = None) -> Address:
        ...
        # Street
        street_value, street_conf = self.find_field(
            forms, self.FIELD_MAPPINGS["street"]
        )
        street_source = "form"
        if not street_value and payload_data:
            street_value = self.get_from_payload(
                payload_data, ["prescriber_street", "street"]
            )
            if street_value:
        ...
        if not city_value and payload_data:
            city_value = self.get_from_payload(
                payload_data, ["prescriber_city", "city"]
            )
            if city_value:
                city_source = "payload"
                city_conf = 1.0
        ...
        if not state_value and payload_data:
            state_value = self.get_from_payload(
                payload_data, ["prescriber_state", "state"]
            )
            if state_value:
