...
class CosentyxFormProcessor:
    ...
    def process_document(
        self, document_bytes: bytes, payload_data: Optional[Dict] = None
    ) -> ExtractionResult:
        ...
        try:
            ...
            result.patient = self.extractors["patient"].extract(
                textract_data, payload_data
            )
            result.prescriber = self.extractors["prescriber"].extract(
                textract_data, payload_data
            )
            result.prescription = self.extractors["prescription"].extract(
                textract_data, payload_data
            )
            result.attestation = self.extractors["attestation"].extract(
                textract_data, payload_data
            )
            ...
            if settings.enable_duplicate_check:
                ...
                if is_duplicate:
                    result.warnings.append(
                        f"Potential duplicate submission: {signature}"
                    )
