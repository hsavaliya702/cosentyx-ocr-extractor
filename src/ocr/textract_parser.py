...
class TextractParser:
    ...
    def find_field_value(
        forms: Dict[str, str], field_names: List[str], case_sensitive: bool = False
    ) -> Optional[str]:
        ...
        for field_name in field_names:
            ...
            if not case_sensitive:
                ...
                for key, value in forms.items():
                    if (
                        field_name_lower in key.lower()
                        or key.lower() in field_name_lower
                    ):
                        return value
    ...
    def extract_field_with_confidence(
        blocks: List[Dict], field_text: str
